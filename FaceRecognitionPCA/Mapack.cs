...
namespace Mapack
{
	...
	public interface IMatrix
	{
		...
		/// <summary>
		/// Calculates element wise multiplication of a matrix
		/// </summary>
		/// <param name="right"></param>
		/// <returns></returns>
		IMatrix HadamardProduct(IMatrix right);
		...
		/// <summary>
		/// Calculates the mean across rows (axis = 0) or across columns (axis = 1)
		/// </summary>
		/// <param name="axis"></param>
		/// <returns></returns>
		IMatrix Mean(int axis);


		/// <summary>
		/// Adds a vector row-wise (axis = 0) or column-wise (axis = 1)
		/// </summary>
		/// <param name="vector"></param>
		/// <param name="axis"></param>
		/// <returns></returns>
		IMatrix AddVector(IMatrix vector, int axis);

		/// <summary>
		/// Adds an array either row-wise (axis = 0) or column-wise (axis = 1)
		/// </summary>
		/// <param name="arr"></param>
		/// <param name="axis"></param>
		/// <returns></returns>
		IMatrix AddVector(double[] arr, int axis);
		...
		/// <summary>
		/// Flattens the matrix into a regular array
		/// </summary>
		/// <returns></returns>
		double[] Flatten();
		...
		/// <summary>
		/// Get's a column from the Matrix
		/// </summary>
		/// <param name="col"></param>
		/// <returns></returns>
		double[] GetColumnAsArray(int col);
	...
	public class Matrix : IMatrix
	{
		...
		*/
		...
		public IMatrix HadamardProduct(IMatrix B)
		{
		...
		/// <summary>
		/// Similar to Numpy Mean method with the caveat that the dimension of the 
		/// Matrix will either be a vertical vector (axis = 1) or a horizontal vector (axis = 0)
		/// </summary>
		/// <param name="axis"></param>
		/// <returns></returns>
		public IMatrix Mean(int axis = 0)
		{
			int firstDimension = this.columns;
			int secondDimension = this.rows;
			Matrix Result = new Matrix(rows: 1, columns: firstDimension);
			if (axis == 1)
			{
				firstDimension = this.rows;
				secondDimension = this.columns;
				Result = new Matrix(rows: firstDimension, columns: 1);
			}
			...
			for (int i = 0; i < firstDimension; i++)
			{
				...
				if (axis == 0)
				{
					Result[0, i] = sum / secondDimension;
				}
				else
				{
					Result[i, 0] = sum / secondDimension;
				}
			}
		...
		private void AddVerticalVector(IMatrix Result, IMatrix vector)
		{
			int leadingDimension = vector.Rows >= vector.Columns ? vector.Rows : vector.Columns;
			if (leadingDimension != this.rows)
			{
				throw new ArgumentException("Dimensions to not match!");
			}
			if (leadingDimension == vector.Columns)
			{
			...
			for (int j = 0; j < columns; j++)
			{
				for (int i = 0; i < rows; i++)
				{
					Result[i, j] = this.data[i][j] + vector[i, 0];
				}
			}
		}

		private void AddHorizontalVector(IMatrix Result, IMatrix vector)
		{
			int leadingDimension = vector.Rows >= vector.Columns ? vector.Rows : vector.Columns;
			if (leadingDimension != this.columns)
			{
				throw new ArgumentException("Dimensions to not match!");
			}
			if (leadingDimension == vector.Rows)
			{
				vector = vector.Transpose();
			}
			...
			for (int i = 0; i < this.rows; i++)
			{
				...
				{
					Result[i, j] = this.data[i][j] + vector[0, j];
				}
			}
		}

		public IMatrix AddVector(IMatrix vector, int axis = 0)
		{
			Matrix Result = new Matrix(this.rows, this.columns);
			if (axis == 0)
			{
				AddHorizontalVector(Result, vector);
			}
			else
			{
				AddVerticalVector(Result, vector);
			}

			return Result;
		}

		public IMatrix AddVector(double[] arr, int axis = 0)
		{
			double[][] data = new double[1][];
			data[0] = arr;
			IMatrix vector = new Matrix(data);
			return AddVector(vector, axis);
		}


		public IMatrix Negate()
		{
			Matrix X = new Matrix(this.rows, this.columns);
			for(int i = 0; i < this.rows; i++)
			{
				for (int j = 0; j < this.columns; j++)
				{
		...
		{
			double[] res = new double[this.columns * this.rows];
			int k = 0;
			for(int i = 0; i < rows; i++)
			{
				for(int j = 0; j < columns; j++)
